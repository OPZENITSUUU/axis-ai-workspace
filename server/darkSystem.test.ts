...
describe("AXIS premium dark visual system", () => {
  it("uses a switchable dark-first theme and includes reduced-motion safeguards", async () => {
    ...
    expect(home).toContain('aria-label={backgroundMode ? "Background task for AXIS" : "Message your AXIS assistant"}');
    ...
    expect(home).toContain("Private Voice Note");
    expect(home).toContain('aria-label="Voice Note"');
    expect(home).toContain('capture="environment"');
    expect(home).toContain("assistantMode");
    expect(home).toContain("axis-request-expo-push");
    expect(home).toContain("axis-expo-push-token");
